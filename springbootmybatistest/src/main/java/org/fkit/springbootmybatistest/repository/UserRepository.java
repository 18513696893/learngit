...
public interface UserRepository {
	@Insert("insert into tb_user (login_name,username,passWord)"
			+"value(#{loginName},#{username},#{password})")
	...
	//@Results用于映射对象属性和数据库列，常用于对象属性和数据库列名不同的情况
	...
	@Update("update tb_user set username=#{username},login_name=#{loginName},password=#{password}"
			+ " where id =#{id}")
